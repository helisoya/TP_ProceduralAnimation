...
void createBuffer3D(Buffer3D& buffer, const CreateBuffer3DParams& params) {
	...
	if (params.pSkinningIndices) {
		glGenBuffers(1, &buffer.vbos[Buffer3D::BufferAttribBoneIndices]);
		glBindBuffer(GL_ARRAY_BUFFER, buffer.vbos[buffer.BufferAttribBoneIndices]);
		glEnableVertexAttribArray(Buffer3D::BufferAttribBoneIndices);
		{
			constexpr size_t size = sizeof(*params.pSkinningIndices) / sizeof((*params.pSkinningIndices)[0]);
			constexpr size_t stride = sizeof(*params.pSkinningIndices);
			glVertexAttribPointer(Buffer3D::BufferAttribBoneIndices, size, GL_FLOAT, GL_FALSE, stride, (void*)0);
		}
		glBufferData(GL_ARRAY_BUFFER, params.vertexCount * sizeof(*params.pSkinningIndices), params.pSkinningIndices, GL_STATIC_DRAW);
	}
	else {
		buffer.vbos[Buffer3D::BufferAttribBoneIndices] = 0;
	}

	if (params.pSkinningWeights) {
		glGenBuffers(1, &buffer.vbos[Buffer3D::BufferAttribBoneWeights]);
		glBindBuffer(GL_ARRAY_BUFFER, buffer.vbos[buffer.BufferAttribBoneWeights]);
		glEnableVertexAttribArray(Buffer3D::BufferAttribBoneWeights);
		{
			constexpr size_t size = sizeof(*params.pSkinningWeights) / sizeof((*params.pSkinningWeights)[0]);
			constexpr size_t stride = sizeof(*params.pSkinningWeights);
			glVertexAttribPointer(Buffer3D::BufferAttribBoneWeights, size, GL_FLOAT, GL_FALSE, stride, (void*)0);
		}
		glBufferData(GL_ARRAY_BUFFER, params.vertexCount * sizeof(*params.pSkinningWeights), params.pSkinningWeights, GL_STATIC_DRAW);
	}
	else {
		buffer.vbos[Buffer3D::BufferAttribBoneWeights] = 0;
	}
