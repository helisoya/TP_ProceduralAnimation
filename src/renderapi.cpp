...
void RenderApi3D::horizontalSkinnedCylinder(const glm::vec3& center, float length, float radius, unsigned int SliceCount, unsigned int StackCount, const glm::vec4& color, GetVertexSkinningWeight* SkinningCallBack, void* pData) const {
	if (SkinningCallBack == nullptr) {
		return;
	}
	unsigned int vertexCount = SliceCount * (StackCount +1);

	glm::vec3* vertices = (glm::vec3*)Allocator.Allocate(sizeof(glm::vec3) * vertexCount);
	glm::vec3* normals = (glm::vec3*)Allocator.Allocate(sizeof(glm::vec3) * vertexCount);
	glm::vec4* colors = (glm::vec4*)Allocator.Allocate(sizeof(glm::vec4) * vertexCount);
	glm::vec4* weights = (glm::vec4*)Allocator.Allocate(sizeof(glm::vec4) * vertexCount);
	glm::ivec4* bonesIndices = (glm::ivec4*)Allocator.Allocate(sizeof(glm::ivec4) * vertexCount);

	unsigned int indiceCount = SliceCount * StackCount * 6;
	unsigned int* indices = (unsigned int*)Allocator.Allocate(sizeof(unsigned int) * indiceCount);

	float fStepZ = length / StackCount;
	float fStepAngle = 2.f * 3.14159f / SliceCount;

	for (unsigned int iVertexAngle = 0; iVertexAngle < SliceCount; ++iVertexAngle) {
		float Cos = cos(iVertexAngle * fStepAngle);
		float Sin = sin(iVertexAngle * fStepAngle);
		for (unsigned int iVertexZ = 0; iVertexZ <= StackCount; ++iVertexZ) {
			unsigned int Indice = iVertexZ * SliceCount + iVertexAngle;
			vertices[Indice] = { center.x + Sin * radius, center.y + Cos * radius, center.z + iVertexZ * fStepZ };
			normals[Indice] = { Sin, Cos, 0.f };
			colors[Indice] = color;
			SkinningCallBack(pData, vertices[Indice], bonesIndices[Indice], weights[Indice]);
		}
	}

	int IndiceCount = 0;
	for (unsigned int iStack = 0; iStack < StackCount; ++iStack) {
		for (unsigned int iSlice = 0; iSlice < SliceCount; ++iSlice) {
			int NextSlice = (iSlice + 1) % SliceCount;
			int FirstCurRow = iStack * SliceCount + iSlice;
			int NextCurRow = iStack * SliceCount + NextSlice;
			int FirstNextRow = (iStack+1) * SliceCount + iSlice;
			int NextNextRow = (iStack+1) * SliceCount + NextSlice;

			indices[IndiceCount++] = FirstCurRow;
			indices[IndiceCount++] = FirstNextRow;
			indices[IndiceCount++] = NextCurRow;
			indices[IndiceCount++] = FirstNextRow;
			indices[IndiceCount++] = NextNextRow;
			indices[IndiceCount++] = NextCurRow;
		}
	}
	...
	createCubeBufferParams.pNormals = normals;
	createCubeBufferParams.pColors = colors;
	createCubeBufferParams.pSkinningWeights = weights;
	createCubeBufferParams.pSkinningIndices = bonesIndices;
