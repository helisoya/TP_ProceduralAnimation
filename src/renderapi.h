...
using GetVertexSkinningWeight = void(void* pData, glm::vec3 const& VertexPos, glm::ivec4& BoneIndices, glm::vec4& BoneWeights);
...
struct RenderApi3D {
	...
	void horizontalSkinnedCylinder(const glm::vec3& center, float length, float radius, unsigned int SliceCount, unsigned int StackCount, const glm::vec4& color, GetVertexSkinningWeight* SkinningCallBack, void* pData) const;
};
