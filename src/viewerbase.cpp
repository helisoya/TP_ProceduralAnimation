...
namespace {
	...
	void render3DSkinningCallback(const RenderApi3D& api, void* pUserData) {
		const Viewer& viewer = *reinterpret_cast<Viewer const*>(pUserData);
		viewer.render3D_Skinning(api);
	}
...
Viewer::Viewer(char const* initialWindowName, int initialViewportWidth, int initialViewportHeight) {
	...
	lightDir = glm::vec4(0.1f, 0.3f, 0.2f, 1.f);
	lightStrength = 0.65f;
	lightAmbient = 0.16f;
	specular = 0.15f;
	specularPow = 30.f;
...
#define ERROR(_ERROR_)\
  ...
  return -1;
	...
	while (!glfwWindowShouldClose(window) && (glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS)) {
		...
		double deltaTime = 0.016666; // 16ms
		if (prevTime != 0) {
			deltaTime = (currentTime - prevTime) / double(CLOCKS_PER_SEC);
		}
		...
		renderParams.render3DSkinningCallback = render3DSkinningCallback;
		renderParams.pRender3DSkinningCallbackUserData = this;
		...
		renderParams.lightDirection = lightDir;
		renderParams.lightStrength= lightStrength; 
		renderParams.lightAmbient = lightAmbient;
		renderParams.specular = specular;
		renderParams.specularPow = specularPow;
		...
		renderParams.pSkinningVertShaderData = pSkinningShaderData;
		renderParams.SkinningVertShaderDataSize = SkinningShaderDataSize;
